...
ITESS_METHOD                    ! OPTIONAL: general (n,m) tesseral method
```
...
| `ITESS_METHOD` | `0` | General (n,m) tesseral force: 0=Cunningham (default; also used if this line is absent — every pre-#30-cross-check `input.dat` keeps working unchanged), 1=LegendreTess (`src/LegendreTess.F`, issue #30's independent cross-check — verified to produce identical propagated output) |
