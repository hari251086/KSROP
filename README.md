...
| 2026-06-21 | Refactored constants initialisation: added `init_constants` subroutine to `Subrouts.F` that reads `input/const_new.dat` and populates the `/xy/` common block (pi, d2r, r2d, amue, AU, R_Earth). All programs (`tle2opm.F`, `test_bugs.F`, `test_subrouts.F`, `test_tle2opm.F`) now call `init_constants()` instead of hardcoding values — eliminates duplication and ensures consistency with `const_new.dat`. Fixed inconsistent common block lengths (4 vs 6 elements) in `car2oe`/`oe2car`; `atan3` now uses the common block instead of recomputing pi. TLEread.F SGP4 WGS-72 `parameter` constants remain unchanged (model-specific). |
